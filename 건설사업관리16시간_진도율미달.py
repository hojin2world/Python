...
def get_configured_driver(download_directory):
    ...
    file_name = f"{today_date}_건설사업관리16시간_진도율미만.xlsx"
...
new_file_path = downloaded_file_path.replace('download.xls', f'{today_date}_건설사업관리16시간진도율.xlsx')
...
# 변경 후 파일 경로
new_file_path = os.path.join(download_directory, f'{today_date}_건설사업관리16시간진도율.xlsx')
...
# 엑셀 파일 경로
excel_file = os.path.join(download_directory, f'{today_date}_건설사업관리16시간진도율.xlsx')
...
new_excel_file = rf'{download_directory}\{today_date}_뿌리오_건설사업관리16시간진도율.xlsx'
...
현 시점 기준 집체교육 참여일까지 수료조건을 모두 만족할 수 있는지 일정 확인하시기 바랍니다.
...
# 파일 경로 설정
file_path = rf"C:\Users\user\Downloads\python\{today_date}_뿌리오_건설사업관리16시간진도율.xlsx"
print(file_input)
print(file_path)
