...
def get_configured_driver(download_directory):
    ...
    file_name = f"{today_date}_건설사업관리16시간시험미응시.xlsx"
...
new_file_path = downloaded_file_path.replace('download.xls', f'{today_date}_건설사업관리16시간시험미응시.xlsx')
...
# 변경 후 파일 경로
new_file_path = os.path.join(download_directory, f'{today_date}_건설사업관리16시간시험미응시.xlsx')
...
print("시험미응시 파일 다운로드 및 형식변환 완료")

################################### 뿌리오 형식으로 변경###################################


# 엑셀 파일 경로
excel_file = os.path.join(download_directory, f'{today_date}_건설사업관리16시간시험미응시.xlsx')
...
new_excel_file = rf'{download_directory}\{today_date}_뿌리오_건설사업관리16시간시험미응시.xlsx'
# 새로운 엑셀 파일 경로 및 파일 이름
df.to_excel(new_excel_file, index=False)  # index=False로 설정하여 인덱스 열을 포함하지 않음

print("시험미응시 파일 뿌리오 변환 완료")
...
# 파일 경로 설정
file_path = rf"C:\Users\user\Downloads\python\{today_date}_뿌리오_건설사업관리16시간시험미응시.xlsx"
print(file_input)
print(file_path)
